...
#函数学习总结：
     # 定义和调用：使用关键字def来定义一个函数，定义函数后需要在函数名的括号中
     #写上参数，最后加： 再换行输入函数内部代码
 '''    def functionname(params):
         staticmethod
         staticmethod'''
     #写一个函数接受一个字符串和一个字母作为参数，并将字符串中出现的字母
     #的数量作为返回值
     def char_count(str,char):
         return str.count(char)
#return关键字，把str 中的 包含char的次数返回给调用者

     #接受一个参数，并将所有的字母及出现的频次都打印出来
     def char_count(str):
         char_list = set(str)
         for char in char_list:
             print(char,str.count(char))
     ...
     #变量作用域： 局部变量和全局变量
     #使用global关键字，对函数中的a标志为全局变量
     #让函数内部使用全局变量 global 关键字，那么整个程序中出现全局变量的a

     #end = '' 表示 print打印后的结尾不用换行，而用空格

     #函数的参数：
         #必选参数（位置参数）：使用函数时必选参数可以不写参数名，但必须对其赋值。
         #参数赋值时定义的时候指定的，所以必选参数又称为位置参数。
         #如果不按照定义顺序传参，就要使用参数名进行传参。

         #注意：必选参数默认顺序必须使用；
               #使用函数时，必选参数顺序错误，但使用了参数名传参。

         #默认参数：函数的参数可以设定默认值，这种参数称为默认参数。
         #调用函数时，如果对默认参数没有赋值，则会自动赋其默认值。
        ''' def connect(ipaddress,port=22):
             print(ipaddress)
             print(port)'''

        #注意点：1.默认参数后面不能再有必选参数，例如：f(a,b=90,c);
            #   2.默认参数的默认值必须设为不可变的数据类型（如
            #   字符串、 元组、数字、布尔值、None 等
        ...
        #可变参数：需要传入的参数 数量不固定，可能是0个、1个...N个
        #这种情况可以为函数设置一个可变参数。
        #可变参数的定义格式： 在参数名前加上 * ，参数名可以自定义，通常写成
        #*args.

        #注意在函数的内部使用该参数时，前面不要加*。
        #调用含可变参数的函数时，如果在可变参数的位置传入多个参数而不是元组，那么这些参数会自动生成一个元组传入函数。


        #关键字参数：关键字参数允许传入零个或多个带参数名的参数，其中参数名可自定义
        #这些关键字参数的定义格式是在参数名前加上** 参数名可以自定义，通常写成 **kw

        #注意在函数体内部使用该参数时，前面不要加 **

        #关键字参数传参时可以使用字典（注意 字典的key值数据类型必须是字符串）
        ...
        #命名关键字参数：赋值时必须写上参数名。
        #次参数特征 前面用一个逗号隔开 的 *。
        # def test(m,*,a,b) 其中m 为必选参数，a和b 就是命名关键字参数。
        #用户调用函数时，每一个命名关键字参数都必须使用相应的参数名赋值。
        #hello( name= 'shiyanlou')

        #函数中修改参数值：python函数的参数是没有类型的，可以传递任意类型的对象作为参数。
        #不同类型的参数在函数中，有都可以修改（列表），有的不可以修改（字符串对象）
